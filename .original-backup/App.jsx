...
const LegalView = ({ type, onClose }) => {
  const content = type === 'privacy' ? {
    ...
    date: 'March 24, 2026',
    sections: [
      { h: '1. Information We Collect', p: 'TaxSense is a privacy-first application. Your financial data is securely stored locally on your device by default. If you opt-in to Cloud Sync, an encrypted payload is stored on Google Firebase to sync across devices. We also collect anonymous diagnostic data to monitor app health.' },
      { h: '2. Third-Party Services', p: 'We use Google Firebase for authentication and optional data sync, and Apple RevenueCat to securely validate your App Store subscription purchases. We strictly do not sell your personal information.' },
      { h: '3. Your Data Rights (GDPR/UK DPA)', p: 'You possess the right to access, rectify, or completely erase your personal data at any point using the "Delete Account" function in Settings, which permanently scrubs your data from Firebase servers.' },
      ...
      { h: '5. Contact', p: 'For any privacy inquiries, please contact our Data Protection Officer at info@taxsense.app.' }
    ]
  } : {
    ...
    sections: [
      ...
      { h: '4. User Data', p: 'Your entries remain encrypted on your device unless you opt-in to Cloud Sync. You are responsible for maintaining the confidentiality of your account credentials.' },
      { h: '5. Limitation of Liability', p: 'To the fullest extent permitted by UK law, TaxSense Accountants Limited shall not be liable for any indirect or consequential damages, including underpayment of tax or financial penalties, resulting from your use of the App.' },
      { h: '6. Governing Law', p: 'These Terms shall be governed by the laws of England and Wales. Contact us at info@taxsense.app.' }
    ]
  };
...
function App() {
      ...
      if (user) {
        try {
          ...
          if (snap.exists()) {
            ...
            const local = localStorage.getItem('taxSenseData_Profiles') || localStorage.getItem('taxTrackerDataV14_Profiles');
            if (local) {
          ...
          // Identify in RevenueCat for cleaner dashboard
          if (user.email) {
            PurchaseService.identifyUser(user.email);
          } else {
            PurchaseService.identifyUser(user.uid);
          }
  ...
  const handleLogout = async () => {
    try {
      ...
      // 2. Sign out of Firebase and RevenueCat
      await signOut(auth);
  ...
  const exportToCSV = async () => {
    ...
    const filename = `TaxTracker_EOY_Export_${taxYear.replace('/', '-')}.csv`;
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
  ...
  if (!currentUser) return <><AuthModal onShowLegal={setLegalView} />{legalView && <LegalView type={legalView} onClose={() => setLegalView(null)} />}</>;
